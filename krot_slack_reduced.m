function [U,P,slack] = krot_slack_reduced(u,A,tol,min_depth,max_depth)
% Solves the known rotation problem with L1-min and slack variables using a
% dimension reduced formualtion, details see:
...
B(3:6:end,:) =  b0+tol*c0;
B(4:6:end,:) = -b0+tol*c0;
B(5:6:end,:) = c0-min_depth;
B(6:6:end,:) = max_depth-c0;

%Add slack variables
C = [B; sparse(ma,1)]; 
B = [sparse(na,1); ones(ma,1)];  
J = kron(speye(ma),ones(n_elc,1));
A = [A, -J; sparse(ma,na), -speye(ma)];
